...
type fonteCSV struct {
	...
	colunas   []string
	registros [][]string
	consumo   cenario.PoliticaDeConsumo
...
func abrirCSV(fonte cenario.FonteDeDados, raiz string) (Fonte, error) {
...
func (f *fonteCSV) Proximo(usuarioVirtual int64) (map[string]string, error) {
	...
	switch f.consumo {
	case cenario.ConsumoAleatorio:
		f.mutex.Lock()
		indice = f.aleatorio.Int63n(total)
		f.mutex.Unlock()
	case cenario.ConsumoUnicoPorUsuario:
		indice = usuarioVirtual % total
	case cenario.ConsumoSequencial:
...
func novaFonteSintetica(fonte cenario.FonteDeDados) (Fonte, error) {
