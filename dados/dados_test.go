...
func TestConsumoCircularVoltaAoInicio(t *testing.T) {
	raiz := escreverCSV(t)
	fonte, err := dados.Abrir(cenario.FonteDeDados{Nome: "assinantes", Arquivo: "assinantes.csv",
		Consumo: cenario.ConsumoCircular}, raiz)
	if err != nil {
...
func TestConsumoSequencialAvisaQuandoOsDadosAcabam(t *testing.T) {
	raiz := escreverCSV(t)
	fonte, _ := dados.Abrir(cenario.FonteDeDados{Nome: "assinantes", Arquivo: "assinantes.csv",
		Consumo: cenario.ConsumoSequencial}, raiz)
...
func TestConsumoUnicoPorUsuarioDaLinhaFixaParaCadaUsuario(t *testing.T) {
	raiz := escreverCSV(t)
	fonte, _ := dados.Abrir(cenario.FonteDeDados{Nome: "assinantes", Arquivo: "assinantes.csv",
		Consumo: cenario.ConsumoUnicoPorUsuario}, raiz)
...
func TestConsumoAleatorioComMesmaSementeRepeteASequencia(t *testing.T) {
	...
	sequencia := func() []string {
		fonte, _ := dados.Abrir(cenario.FonteDeDados{Nome: "assinantes", Arquivo: "assinantes.csv",
			Consumo: cenario.ConsumoAleatorio, Semente: 42}, raiz)
		var lidos []string
		for i := 0; i < 10; i++ {
...
func TestGeracaoSinteticaEhReproduzivelPelaSemente(t *testing.T) {
	fonte := cenario.FonteDeDados{Nome: "pedidos", Semente: 7, Campos: map[string]string{
		"id":     "uuid",
		"valor":  "numero(10,500)",
		"ordem":  "sequencia",
		"quem":   "nome",
...
func TestGeradorDesconhecidoEnsinaOsValidos(t *testing.T) {
	fonte := cenario.FonteDeDados{Nome: "x", Campos: map[string]string{"a": "telefone"}}
	aberta, err := dados.Abrir(fonte, "")
	if err != nil {
...
func TestArquivoInexistenteExplicaOProblema(t *testing.T) {
	_, err := dados.Abrir(cenario.FonteDeDados{Nome: "x", Arquivo: "nao-existe.csv"}, t.TempDir())
