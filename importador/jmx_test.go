...
func TestImportaJMXEGeraCenarioQueOParserAceita(t *testing.T) {
	importacao, err := importador.DeJMX([]byte(planoDeJMeter))
	if err != nil {
		t.Fatalf("nao importou: %v", err)
	}

	c, err := cenario.Carregar([]byte(importacao.YAML))
	if err != nil {
...
func TestJMXSemRequisicaoHTTPExplicaOQueOImportadorCobre(t *testing.T) {
	_, err := importador.DeJMX([]byte(`<?xml version="1.0"?><jmeterTestPlan><hashTree><TestPlan testname="vazio"/></hashTree></jmeterTestPlan>`))
	if err == nil {
