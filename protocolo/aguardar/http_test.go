...
func configuracaoHTTP(t *testing.T, corpo string) protocolo.Configuracao {
	t.Helper()
...
`)
	...
	inicio := time.Now()
	resposta := aguardar.Novo(protocolo.OpcoesPadrao()).Executar(context.Background(), protocolo.Requisicao{
...
`)
	...
	if resposta.Classe != protocolo.ErroDeTimeout {
