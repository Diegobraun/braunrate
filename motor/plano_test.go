...
func planoDe(fases ...cenario.Fase) motor.Plano {
	return motor.CompilarPlano(cenario.PlanoDeCarga{Modelo: cenario.ChegadaAberta, Fases: fases})
}

func TestTaxaConstanteAgendaEmIntervaloFixo(t *testing.T) {
	plano := planoDe(cenario.Fase{Tipo: cenario.FaseConstante, Ate: 100, Durante: 2 * time.Second})
...
func TestRampaAgendaPelaIntegralDaTaxa(t *testing.T) {
	plano := planoDe(cenario.Fase{Tipo: cenario.FaseRampa, De: 0, Ate: 100, Durante: 10 * time.Second})
...
func TestPerfisEmSequenciaSomamDuracaoEQuantidade(t *testing.T) {
	plano := planoDe(
		cenario.Fase{Tipo: cenario.FaseRampa, De: 0, Ate: 100, Durante: 2 * time.Second},
		cenario.Fase{Tipo: cenario.FasePatamar, Ate: 100, Durante: 3 * time.Second},
		cenario.Fase{Tipo: cenario.FasePico, Ate: 500, Durante: 1 * time.Second},
	)
...
func TestInstantesSaoMonotonicos(t *testing.T) {
	plano := planoDe(
		cenario.Fase{Tipo: cenario.FaseRampa, De: 10, Ate: 200, Durante: 3 * time.Second},
		cenario.Fase{Tipo: cenario.FasePatamar, Ate: 200, Durante: 2 * time.Second},
	)
