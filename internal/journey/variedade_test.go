...
func TestTodoValorDeclaradoChegaAoAlvo(t *testing.T) {
	...
	porNome := map[string]metrics.Variedade{}
...
func TestVariedadeDeUmValorSoViraAvisoGrave(t *testing.T) {
	documento := metrics.Documento{
		Variedade: []metrics.Variedade{
			{Nome: "assinantes.id", Distintos: 1, Usos: 2375, Disponivel: 3},
		},
	}
	avisos := metrics.AvisosDeVariedade(documento.Variedade)
	if len(avisos) != 1 {
		t.Fatalf("esperava um aviso, obtive %+v", avisos)
	}
	if avisos[0].Gravidade != metrics.GravidadeAlta {
...
func TestValorFixoDeclaradoNoCenarioEhAvisoDeLeituraENaoDefeito(t *testing.T) {
	avisos := metrics.AvisosDeVariedade([]metrics.Variedade{
		{Nome: "pedidoFixo", Distintos: 1, Usos: 500, Disponivel: 0},
	})
	if len(avisos) != 1 || avisos[0].Gravidade != metrics.GravidadeMedia {
...
func executarCenarioDeVariedade(t *testing.T, endereco, csv string) metrics.Documento {
	t.Helper()
	raiz := t.TempDir()
