...
func Comparacao(saida io.Writer, c comparison.Comparacao) {
	...
	escrever("  Duas execucoes nao dao intervalo de confianca: variacao abaixo de %.0f%% e tratada como ruido.", comparison.RuidoAceito*100)
	escrever("")
}

func variacao(diferenca comparison.Diferenca) string {
	if diferenca.Sentido == comparison.SentidoIgual {
		return "ruido"
	}
	magnitude := strings.Replace(comparison.Magnitude(diferenca), " vezes", "x", 1)
	if diferenca.Sentido == comparison.SentidoMelhor {
