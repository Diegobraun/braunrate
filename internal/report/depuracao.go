...
func Depuracao(saida io.Writer, numero int, observacao engine.Observacao, mostrarCorpo bool) {
	...
	if observacao.Classe != protocol.Sucesso && len(observacao.Falhas) == 0 {
...
func descreverConfiguracao(configuracao protocol.Configuracao) []string {
	if configuracao == nil {
		return []string{"(nao montada)"}
	}
	if descritivel, sabe := configuracao.(protocol.ConfiguracaoDescritivel); sabe {
