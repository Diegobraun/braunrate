...
func HTML(saida io.Writer, documento metrics.Documento) error {
	pagina := montarPagina(documento)
	return modeloHTML.Execute(saida, pagina)
}

func montarPagina(documento metrics.Documento) paginaHTML {
	pagina := paginaHTML{
		Documento: documento,
	...
	for _, aviso := range documento.Avisos {
		...
		case metrics.GravidadeAlta:
			linha.Classe, linha.Rotulo = "alta", "resultado invalido"
		case metrics.GravidadeMedia:
...
func montarVeredito(documento metrics.Documento) vereditoHTML {
...
func frasearVolume(documento metrics.Documento) string {
...
func frasesDeConfiabilidade(documento metrics.Documento) []string {
	var frases []string
...
func frasesDoPlano(documento metrics.Documento) []string {
	var frases []string
...
func frasesDoAmbiente(documento metrics.Documento) []string {
	frases := []string{
...
func desenharSeries(series []metrics.Bucket) (template.HTML, bool) {
	if len(series) < 2 {
		return "", false
	}
	ordenadas := append([]metrics.Bucket{}, series...)
	...
	linha := func(escolher func(metrics.Bucket) float64) string {
		var pontos []string
	...
	fmt.Fprintf(&svg, `<polyline class="p99" points="%s"/>`, linha(func(b metrics.Bucket) float64 { return b.LatenciaP99Ms }))
	fmt.Fprintf(&svg, `<polyline class="p50" points="%s"/>`, linha(func(b metrics.Bucket) float64 { return b.LatenciaP50Ms }))
