...
func Extrair(captura scenario.Captura, resposta protocol.Resposta) (string, error) {
	switch captura.Origem {
	case scenario.CapturaDeJSON:
		return extrairDeJSON(captura, resposta)
	case scenario.CapturaDeCabecalho:
	...
	case scenario.CapturaDeStatus:
		return strconv.Itoa(resposta.Status), nil
	case scenario.CapturaDeCorpo:
...
func extrairDeJSON(captura scenario.Captura, resposta protocol.Resposta) (string, error) {
