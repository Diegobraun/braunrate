...
func decodificar(t *testing.T, texto string) (protocol.Configuracao, error) {
	...
	return wait.Novo(protocol.OpcoesPadrao()).Decodificar(documento.Content[0])
}
...
func TestTimeoutTemPadraoEhPodeSerDeclarado(t *testing.T) {
	...
	if !strings.Contains(strings.Join(comTimeout.(protocol.ConfiguracaoDescritivel).Descrever(), " "), "1m30s") {
...
func TestSemEnderecoEnsinaOndeDeclarar(t *testing.T) {
	...
	resposta := wait.Novo(protocol.OpcoesPadrao()).Executar(t.Context(), protocol.Requisicao{Configuracao: configuracao})
	if resposta.Classe != protocol.ErroDeConfigacao || !strings.Contains(resposta.Detalhe, "brokers") {
