...
func (broker *Broker) mechanism() (sasl.Mechanism, error) {
	...
	return nil, fmt.Errorf("tipo de autenticacao desconhecido: %q", broker.Auth.Kind)
}
...
func (broker *Broker) credentialPresent() error {
	...
	if broker.Auth.PasswordVar != "" {
		return fmt.Errorf("a variavel de ambiente %s nao esta definida, entao a senha do broker ficou vazia: rode com %s=... no ambiente",
			broker.Auth.PasswordVar, broker.Auth.PasswordVar)
	}
	return fmt.Errorf("autenticacao %s sem senha declarada", broker.Auth.Kind)
}
...
func (mechanism mskMechanism) Start(runContext context.Context) (sasl.StateMachine, []byte, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("nao consegui assinar com IAM na regiao %s: %w", mechanism.region, err)
	}
...
func Explain(kind string, broker *Broker) string {
	switch kind {
	case "autenticacao":
		return fmt.Sprintf("o broker recusou a credencial (%s): confira o usuario e a variavel de ambiente com a senha", broker.Describe())
	case "autorizacao":
		return fmt.Sprintf("a credencial foi aceita e nao tem permissao nesse topico ou grupo (%s): e caso de ACL no broker, nao de senha errada", broker.Describe())
	}
	return ""
}
