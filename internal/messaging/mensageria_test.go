...
func executar(t *testing.T, conteudo string) (metrics.Documento, slo.Veredito) {
	t.Helper()
	raiz := t.TempDir()
	...
	t.Cleanup(func() { protocol.EncerrarTodos() })
...
func TestCadeiaAssincronaMedeDoProdutorAoConsumidor(t *testing.T) {
	...
	porNome := map[string]metrics.ResultadoDePasso{}
...
func TestMensagemQueNaoChegaViraTimeoutComExplicacao(t *testing.T) {
	...
	var espera metrics.ResultadoDePasso
	for _, passo := range documento.Passos {
