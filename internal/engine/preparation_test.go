...
func TestPreparationIsPaidBeforeTheClockStarts(t *testing.T) {
	...
	if !document.Valid() {
		t.Fatalf("a preparacao invalidou a medicao: %+v", document.Sanity.Findings)
	}
	...
	if latency := document.Overall.Latency; latency.Max >= float64(slow.cost.Milliseconds()) {
		t.Fatalf("a preparacao caiu dentro da latencia: pior caso %.1f ms contra %s de abertura",
