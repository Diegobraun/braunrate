...
func amostra(agendado time.Time, atrasoDeEnvio, servico time.Duration, classe protocol.ClasseDeErro) metrics.Amostra {
	envio := agendado.Add(atrasoDeEnvio)
	return metrics.Amostra{
...
func TestLatenciaEhContadaDoInstanteAgendado(t *testing.T) {
	a := amostra(inicioFixo, 50*time.Millisecond, 10*time.Millisecond, protocol.Sucesso)
...
func TestPercentilVemDoHistogramaENaoDaMedia(t *testing.T) {
	agregado := metrics.NovoAgregado("passo", "chave", "http")
	for i := 0; i < 99; i++ {
		agregado.Registrar(amostra(inicioFixo, 0, 10*time.Millisecond, protocol.Sucesso))
	}
	agregado.Registrar(amostra(inicioFixo, 0, 5*time.Second, protocol.Sucesso))
...
func TestAgregadosSaoMergeaveis(t *testing.T) {
	primeiro := metrics.NovoAgregado("passo", "chave", "http")
	segundo := metrics.NovoAgregado("passo", "chave", "http")

	for i := 0; i < 500; i++ {
		primeiro.Registrar(amostra(inicioFixo, 0, 10*time.Millisecond, protocol.Sucesso))
	}
	for i := 0; i < 500; i++ {
		segundo.Registrar(amostra(inicioFixo, 0, 100*time.Millisecond, protocol.ErroDeTimeout))
	}

	juntos := metrics.NovoAgregado("passo", "chave", "http")
...
func montarDocumento(c *metrics.Coletor, inicio, fim time.Time) metrics.Documento {
	c.Encerrar()
	return metrics.MontarDocumento(c, metrics.EntradaDoDocumento{
...
func TestBackPressureAcimaDeUmPorCentoInvalidaOResultado(t *testing.T) {
	coletor := metrics.NovoColetor(inicioFixo, 10*time.Millisecond)
	for i := 0; i < 100; i++ {
		...
		coletor.Registrar(amostra(agendado, atraso, 5*time.Millisecond, protocol.Sucesso))
	}
	...
	for _, aviso := range documento.Avisos {
		if aviso.Tipo == "gerador_saturado" && aviso.Gravidade == metrics.GravidadeAlta {
			encontrou = true
...
func TestAmostraPerdidaInvalidaOResultado(t *testing.T) {
	coletor := metrics.NovoColetorComCapacidade(inicioFixo, 10*time.Millisecond, 1)
	for i := 0; i < 200_000; i++ {
		coletor.Registrar(amostra(inicioFixo, 0, time.Millisecond, protocol.Sucesso))
	}
...
func TestSeriesTemporaisUsamBucketAlinhadoAoEpoch(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		coletor.Registrar(amostra(agendado, 0, 5*time.Millisecond, protocol.Sucesso))
	}
