...
type ExecutarPasso func(ctx context.Context, passo scenario.Passo, valores *runtime.Contexto) (protocol.Resposta, error)

type Gerenciador struct {
	configuracao scenario.Autenticacao
...
func Novo(configuracao scenario.Autenticacao, executar ExecutarPasso, relogio Relogio) *Gerenciador {
...
func (g *Gerenciador) Cabecalho(ctx context.Context, valores *runtime.Contexto) (string, string, error) {
	if g.configuracao.Tipo == scenario.AutenticacaoBasica {
...
func (g *Gerenciador) garantirToken(ctx context.Context, valores *runtime.Contexto) error {
	g.mutex.Lock()
	...
	valoresDaObtencao := runtime.Novo(0, 0, entrada)
