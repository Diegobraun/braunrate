...
func prepararCenario(t *testing.T, endereco string) (cenario.Cenario, string) {
	t.Helper()
	raiz := t.TempDir()
...
func executar(t *testing.T) (metrica.Documento, slo.Veredito) {
	t.Helper()
...
func TestAssercaoQuebradaSeparaFalhaFuncionalDeFalhaDeSLO(t *testing.T) {
	...
	c, raiz := prepararCenario(t, servidor.Endereco())
	c.Passos[0].Assercoes = []cenario.Assercao{{
		Tipo: cenario.AsserirJSON, Alvo: "$.ultimaFatura.status",
		Operador: cenario.OperadorIgual, Valor: "PAGA",
	}}

	opcoes := motor.OpcoesPadrao()
	opcoes.RaizDeDados = raiz
	m, _ := motor.Novo(c, opcoes)
	documento := m.Executar(context.Background())
